if (typeof HdGame == "undefined") {
  ...
  HdGame.showMsgToast2 = function(options) {
    ...
    options);
    var str = '<div class="toast" style="z-index: 2000; position: relative; display: none;"><div class="weui-mask" style="z-index: 2000;"></div><div class="weui-dialog" style="z-index: 2000;">';
  ...
  HdGame.refreshGiftListAndAwardDetail = function(gameId, openId, code, award) {
      ...
      creatRankList: function(info) {
        ...
        if (rankList.length == 0) {
          ...
          for (var i = 0; i < rankList.length; i++) {
            ...
            var tr = '<tr class="rankInfo"><td>No.' + (rankAjax.start + i + 1) + '</td><td><div><img class="userImg" ' + srcStr + " />" + otherImg + '</div></td><td class="userName">' + HdGame.encodeHtml(rankList[i].name) + "</td><td >" + rankList[i].achievement + '<span class="scoreUnit">' + g_config.scoreUnit + "</span></td></tr>";
          ...
          if ((rankAjax.isRankAll || rankAjax.hasLoadAll) && rank > rankShowNum) {
            ...
            var srcStr = g_config.headImg ? "src='" + g_config.headImg + "'": "";
            var str = '<tr class="rankInfo" style="line-height:0.85rem"><td colspan="4" style="padding-bottom:0.35rem">......</td></tr><tr class="rankInfo"><td>No.' + rank + '</td><td><div><img class="userImg" ' + srcStr + ' /></div></td><td class="userName">' + HdGame.encodeHtml(g_config.userName) + "</td><td >" + (HdGame.currentScore ? ((g_config.scoreType ? HdGame.currentScore: parseInt(HdGame.currentScore)) + '<span class="scoreUnit">' + g_config.scoreUnit + "</span>") : "无") + "</td></tr>";
      ...
      if (!_manage) {
        ...
        if (skillSupportType == 2 || skillSupportType == 3) {
          if (g_config.authVer == HdVerDef.FREE) {
            HdGame.logDog(1000069, 1)
          } else {
            if (g_config.authVer == HdVerDef.BJ) {
              HdGame.logDog(1000069, 3)
            } else {
              if (g_config.authVer == HdVerDef.BY) {
                HdGame.logDog(1000069, 5)
              }
            }
          }
    ...
    var poupAward = function(flag) {
      ...
      if (!_manage && g_config.showSkillSup) {
        if (g_config.authVer == HdVerDef.FREE) {
          HdGame.logDog(1000069, 0)
        } else {
          if (g_config.authVer == HdVerDef.BJ) {
            HdGame.logDog(1000069, 2)
          } else {
            if (g_config.authVer == HdVerDef.BY) {
              HdGame.logDog(1000069, 4)
            }
          }
        }
      ...
      HdGame.openAwardDetail = function(itemInfo) {
        ...
        if (_manage) {
          ...
          if (awardInfo.genewxcard) {
            ...
            bottomCusBtnBox.hdToggle("codeStatus", !(itemInfo.theGiftDate < 0 || (awardInfo.$type.sitetype == "text" && $.inArray(itemInfo.codeStatus, [4, 1, 5, 3]) != -1)))
          }
        ...
        function(val) {
          awardDetailBox.find("#bottomCusBtnBox,.codeOptInfo").hdToggle("emptyUrl", !val)
        });
        watch("{award}.$opqrc",
        function(val) {
        ...
        var seeFsnDetailBtn = $("#seeFsnDetail").hide().data("codeInfo", itemInfo);
        ...
        if (awardInfo.awardtype == 9) {
          ...
          if (!_manage && isFsnCoupon && !itemInfo.fissileOriginCode) {
            ...
            seeFsnDetailBtn.show();
            if ($(this).hasClass("seeCurAwaFsn") || [1, 3, 4, 5].indexOf(itemInfo.codeStatus) != -1 || curFsnData.fissileFinish) {
              awardDetailBox.show();
              $("#myFissionPage").hide()
            } else {
              HdGame.fission.openMyFsn(itemInfo)
            }
          } else {
            awardDetailBox.show()
          }
  ...
  HdGame.initWxConfig = function(arg) {
    ...
    function setWxShareArg(args) {
      ...
      if (args.haveAward) {
        if (args.plInfo.$awardLevel) {
          setCurrentAward(args.plInfo.$awardLevel.join(""))
        }
      }
    ...
    function setCurrentAward(awardLevel) {
      var currentAward = [];
      ...
      g_config.awardList.map(function(awardItem) {
        if (new RegExp(awardItem.level).test(awardLevel)) {
          ...
          currentAward.push(awardItem.name);
  ...
  HdGame.initJsHead = function(hg, _data) {
    ...
    hg.assets = (function() {
      ...
      function initAdvertisingHTML(pathDef) {
        ...
        var random = -1;
        if (  g_config.advertisingDisplayModel == 1) {
      ...
      var assetsImage = [_data.logoImg_path, _resRoot + "/image/ruleImg.png", _resRoot + "/image/success.png", _resRoot + "/image/light.png", _resRoot + "/image/musicOff.png", _resRoot + "/image/musicOn.png", g_config.headImg];
  ...
  HdGame.initJsFoot = function(arg) {
    ...
    $(function() {
      ...
      if (g_config.style == 75) {
        ...
        $("#resule-gift-sucImg, .clickHere, .seeAwardDetail").on("touchstart",
        function(e) {
          g_config.firstTouchRank = g_config.firstTouchAward = g_config.firstTouchWinList = true;
          showAwardDetail4Draw()
        })
