...
import { Sequelize, GameRound,  GamePlayer, GameResult, GameDay } from '../../models'
...
class Bargain {
    ...
    static async playWx(ctx) {
      ...
      let game_player = await game_players.findOne({ where: { id: game_player_id, game_round_id} } )
      let to_game_player = game_player
      if(to_game_player_id ){
        to_game_player = ( await game_players.findOne({ where: { id: to_game_player_id, game_round_id} })) || game_player
      }
      if( game_player ){

        let game_round = await game_rounds.findById(game_round_id);
        // 检查当前用户的game_day
        let game_day = game_days.findOrCreate({where:{ game_round_id,game_player_id, day: new Date() }})
    ...
    static async checkinWx(ctx) {
      ...
      const current_game = await game_rounds.findById(game_round_id);
      const current_player = await game_players.findOne({ where:{game_round_id, openid}});
      ...
      if( current_player ){
        ...
        let params = { game_round_id: game_round_id,
          openid: ctx.query.openid,
          nickname: ctx.query.nickname, avatar: ctx.query.headimgurl}

          //创建玩家
          context.game_player = await game_players.create( params )
    ...
    static async gameInfoWx( ctx ){
      ...
      let url = ctx.request.body.url // 可能有 to_game_player_id 或者没有

      const game_round = await game_rounds.findById(game_round_id);

      const game_player = await game_players.findById(game_player_id);

      // 参加此活动总人数和排名
      let game_player_rank = await game_players.findAll({ where:{ game_round_id }, order:[['score', 'DESC'], ['id', 'ASC']], limit: 20, offset: 0, })
      ...
      if( to_game_player_id ){// 如果有被助力人
         to_game_player = await game_players.findById(to_game_player_id);
          // 判断是否为本机的活动 我要参与
          // 砍价日志的获取判断是否砍过
          game_result = await game_results.findOne({ where:{ game_player_id, game_round_id, to_game_player_id: to_game_player_id }})
          //$list_log = M("bargain_log")->where(array("user_id" => $user_id, "openid" => $openid, "bargain_id" => $bargain_id))->select();
      }else{
        // 砍价日志的获取判断是否砍过
        game_result = await game_results.findOne({ where:{ game_player_id, game_round_id, to_game_player_id: game_player_id }})
      }
      // 有多少助力成绩
      let game_result_rank = await game_results.findAll({ where:{ game_round_id, to_game_player_id: to_game_player.id }, include: [{ model: game_players }], limit: 20, offset: 0})
      //let giving_game_players = await game_players.findAll({ where:{ game_round_id, id:game_player_id }})
    ...
    static async pollWx( ctx ){
      ...
      let to_game_player_id = ctx.request.body.to_game_player_id //被助力人id，可能为空
      const game_round = await game_rounds.findById(game_round_id);
      let game_player = await game_players.findById(game_player_id);
      let to_game_player = await game_players.findById(to_game_player_id);
      let data = { }
      if( game_player ){
        ...
        if ( to_game_player.score < game_round.final_score ) {
           // 砍完价以后价格低于底价则修改为底价
            ...
            let game_result = await game_results.findOne({ where:{ game_player_id, game_round_id, to_game_player_id: to_game_player_id }})
            if (!game_result ) {
              ...
              const game_result = await game_results.create(result_values, options)
              var player_values = { score: new_score }
              await to_game_player.update( player_values )
              let game_result_rank = await game_results.findAll({ where:{ game_round_id, to_game_player_id: to_game_player.id }, include: [{ model: game_players }], order:[['score', 'DESC'], ['id', 'ASC']], limit: 20, offset: 0, })
    ...
    static async gameResultRank( ctx ){
      ...
      let game_result_rank = await game_results.findAll({ where:{ game_round_id, to_game_player_id: game_player_id }, include: [{ model: game_players }], order:[['score', 'DESC'], ['id', 'ASC']], limit: per_page, offset: 0, })

      ctx.body = { game_result_rank }
    }
    ...
    static async gamePlayerRank( ctx ){
      ...
      let game_player = {}
      if(game_player_id){
        game_player = await game_players.findById( game_player_id )
        //   (score > game_player.score) || (score == game_player.score)
        let position = await game_players.count( { where:{ game_round_id, [Op.or]:{ score: {[Op.gt]: game_player.score}, [Op.and]: { score:game_player.score, id:{[Op.lt]: game_player.id}} }}})
        game_player.rank = position +1
      }
      let game_player_rank = await game_players.findAll({ where:{ game_round_id }, order:[['score', 'DESC'], ['id', 'ASC']], limit: per_page, offset: 0 })
      game_player_rank.forEach((item, i)=>{
        item.rank = i+1
      })
      ctx.body = { game_player_rank, game_player }
    }
    ...
    static async updateGameDayWx( ctx ){
      ...
      let game_player = await game_players.findById(game_player_id);
      // [game_day, boolean]
      let [game_day,is_created] = await game_days.findOrCreate({where:{ game_round_id,game_player_id, day: new Date() }})
      if( share ){
