...
const { Sequelize, game_rounds, games, game_players, game_awards, game_results, game_days } = require('../../app-db/DBMODAL')
...
class GameLog {
     ...
     */
    static async logAjaxErr_h(ctx) {
      let data = {rt: 0}
      ctx.body = data
    }
    static async log_h(ctx) {
      console.log("log_h=", ctx.query )
      let data = {rt: 0}
      ctx.body = data
    }
    static async logJsErr_h(ctx) {
      let data = {rt: 0}
      ctx.body = data
    }
    static async hdgame_h(ctx) {
