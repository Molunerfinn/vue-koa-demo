...
class Bargain {
    ...
    static async playWx(ctx) {
      ...
      let game_player = await GamePlayer.findOne({ where: { id: game_player_id, game_round_id} } )
      let to_game_player = game_player
      if(to_game_player_id ){
        to_game_player = ( await GamePlayer.findOne({ where: { id: to_game_player_id, game_round_id} })) || game_player
      }
      if( game_player ){
...
console.log( "before to_game_player_id")
    ...
    static async gameResultRank( ctx ){
      ...
      let game_result_rank = await GameResult.findAll({ where:{ game_round_id, to_game_player_id: game_player_id }, include: [{ model: GamePlayer }], order:[['score', 'DESC'], ['id', 'ASC']], limit: per_page, offset: 0, })
    ...
    static async gamePlayerRank( ctx ){
      ...
      if(game_player_id){
        ...
        let position = await GamePlayer.count( { where:{ game_round_id, [Op.or]:{ score: {[Op.gt]: game_player.score}, [Op.and]: { score:game_player.score, id:{[Op.lt]: game_player.id}} }}})
        game_player.rank = position +1
      }
      let game_player_rank = await GamePlayer.findAll({ where:{ game_round_id }, order:[['score', 'DESC'], ['id', 'ASC']], limit: per_page, offset: 0 })
    ...
    static async updateGameDayWx( ctx ){
      ...
      let game_player = await GamePlayer.findById(game_player_id);
      // [game_day, boolean]
      let [game_day,is_created] = await GameDay.findOrCreate({where:{ game_round_id,game_player_id, day: new Date() }})
      if( share ){
