...
import { getGameRoundNumber } from './helper'
import PintuRunner from '../../models/dpgame/pintu/runner'
import { DpGameRoundStates } from '../../models/constant'
...
export default class DpPintuSocket{
    ...
    const dynamicNsp = io.of(/^\/channel-dppintu-\w+$/).on('connect', (socket) => {
      ...
      socket.on('StartGameEvent', async ( data, callback) =>{
        ...
        // 这时游戏状态是started
        callback({ gameRoundState: DpGameRoundStates.starting, gameRound: gameRound })
        ...
        var countTimeId = setInterval(function(){
            ...
            if(countTime <= 1){
                ...
                var runningTimeId = setInterval( function(){
                    ...
                    let payload =  { timeToEnd: lastRunningTime, gameRoundState: DpGameRoundStates.started }
            ...
            countTime--;
            // 触发客户端的倒计时事件
            let payload = {gameRoundState: DpGameRoundStates.starting , timeToStart: countTime}
